...
- Backend:
  - For Ruby on Rails, use the built-in test runner, `rails test` and `assert`. Follow `backend-agent/rails/references/testing.md`.
  - For Node.js, use the built-in test runner, `node:test` and `t.assert`. Follow `backend-agent/node/references/testing.md`.
