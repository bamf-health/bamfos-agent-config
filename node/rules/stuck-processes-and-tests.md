...
node --test --test-timeout=15000 path/to/file.test.js
node --test --test-timeout=15000 --test-name-pattern="should close resources" path/to/file.test.js

# 3) Dump active handles when hung
node --import why-is-node-running/include --test path/to/file.test.js
...
for i in {1..30}; do
  "$TIMEOUT_BIN" 30s node --test path/to/file.test.js || { echo "failed on run $i"; break; }
done
```
