...
```javascript
const POPULATED_COMMENT_LIMIT = 20;
...
async function getPost(id, config = {}) {
  ...
  if (config.includeComments) {
    query = query.populate({
      ...
      options: {limit: POPULATED_COMMENT_LIMIT, sort: {createdAt: -1}},
    });
  }
...
.populate('posts')
.populate('postCount');
// author.posts = array of post documents
    ...
    items: [
      {
        ...
        quantity: {type: Number, required: true, min: MIN_QUANTITY},
      },
    ],
